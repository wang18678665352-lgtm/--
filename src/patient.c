...
static int select_doctor_in_department(const char *department_id, Doctor *selected_doc) {
    ...
    printf("\n请输入医生编号: ");
    if (fgets(doctor_id, sizeof(doctor_id), stdin) == NULL) {
...
static void show_unassigned_doctors(void) {
    DoctorNode *head = load_doctors_list();
    DoctorNode *current = head;
    int found = 0;
    ...
    while (current) {
        if (current->data.department_id[0] == '\0') {
            has_unassigned = 1;
            break;
        }
        current = current->next;
    ...
    while (current) {
        if (current->data.department_id[0] == '\0') {
            printf("%-10s %-15s %-10s %-10d\n",
                   ...
                   current->data.name,
                   current->data.title[0] ? current->data.title : "未设置",
...
static int select_department_and_doctor(char *department_id, Doctor *selected_doc) {
    DepartmentNode *department_head = NULL;
    DepartmentNode *current_department = NULL;
    DoctorNode *doctor_head = NULL;
    DoctorNode *current_doctor = NULL;
    char doctor_id[MAX_ID];
    int department_found = 0;

    show_available_departments();
    show_unassigned_doctors();

    printf("\n请输入科室编号(或输入\"ALL\"查看所有未分配科室医生): ");
    if (fgets(department_id, MAX_ID, stdin) == NULL) {
        return ERROR_INVALID_INPUT;
    }
    department_id[strcspn(department_id, "\n")] = 0;

    if (strcmp(department_id, "ALL") == 0) {
        DoctorNode *all_head = load_doctors_list();
        DoctorNode *all_current = all_head;
        ...
        while (all_current) {
            if (all_current->data.department_id[0] == '\0') {
                printf("%-10s %-15s %-10s %-10d\n",
                       all_current->data.doctor_id,
                       all_current->data.name,
                       all_current->data.title[0] ? all_current->data.title : "未设置",
                       all_current->data.busy_level);
            }
            all_current = all_current->next;
        }
        free_doctor_list(all_head);

        printf("\n请输入医生编号: ");
        if (fgets(doctor_id, MAX_ID, stdin) == NULL) {
            return ERROR_INVALID_INPUT;
        }
        doctor_id[strcspn(doctor_id, "\n")] = 0;

        doctor_head = load_doctors_list();
        current_doctor = doctor_head;
        while (current_doctor) {
            if (strcmp(current_doctor->data.doctor_id, doctor_id) == 0 &&
                current_doctor->data.department_id[0] == '\0') {
                *selected_doc = current_doctor->data;
                strcpy(department_id, "未分配");
        ...
        printf("医生编号不存在或不是未分配科室医生!\n");
        return ERROR_NOT_FOUND;
    }

    department_head = load_departments_list();
    current_department = department_head;
    while (current_department) {
        if (strcmp(current_department->data.department_id, department_id) == 0) {
            department_found = 1;
            break;
        }
    ...
    {
        Doctor *recommended_doctor = find_recommended_doctor(department_id);
        if (recommended_doctor) {
            printf("推荐医生: %s (%s, 当前工作量 %d)\n",
                   recommended_doctor->name,
                   recommended_doctor->doctor_id,
                   recommended_doctor->busy_level);
            free(recommended_doctor);
        }
    }
    ...
    while (current_doctor) {
        if (strcmp(current_doctor->data.doctor_id, doctor_id) == 0 &&
            (strcmp(current_doctor->data.department_id, department_id) == 0 ||
             current_doctor->data.department_id[0] == '\0')) {
            *selected_doc = current_doctor->data;
            free_doctor_list(doctor_head);
            return SUCCESS;
